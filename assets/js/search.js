...
function scrollToStory(title) {
	const storyId = title.replace(/\s+/g, "-").toLowerCase();
	const currentPage = window.location.pathname;

	if (currentPage.includes("stories.html")) {
		const selector = `#${CSS.escape(storyId)}`;
		const storyElement = document.querySelector(selector);
		if (storyElement) {
			storyElement.scrollIntoView({ behavior: "smooth" });
			container.classList.remove("active");
		}
	} else {
		// Nếu không phải stories.html thì chuyển sang stories.html và thêm hash
		window.location.href = `stories.html#${storyId}`;
	}
}
