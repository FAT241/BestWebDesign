...
// Hàm chuyển trang (định nghĩa ngoài scope để gọi từ HTML)
...
function handleSearch() {
	const searchInput = document.getElementById("searchInputDonation");
	if (!searchInput) {
		console.error("Không tìm thấy input tìm kiếm với ID 'searchInputDonation'");
		return;
	}

	const keyword = searchInput.value.trim().toLowerCase();
	console.log("Từ khóa tìm kiếm (Danh sách ủng hộ):", keyword);
	...
	filteredDonations = donations.filter((donation) => {
		...
		const du_anMatch = donation.du_an.toLowerCase().replace(/\s+/g, " ").includes(keyword.replace(/\s+/g, " "));
		...
		console.log(`Kiểm tra donation: ${donation.name}, Amount: ${cleanAmount}, Dự án: ${donation.du_an}, Kết quả:`, {
			nameMatch,
			dateMatch,
			du_anMatch,
			costMatch,
	...
	console.log("Danh sách lọc (Danh sách ủng hộ):", filteredDonations);
...
function renderTable(page) {
	...
	if (!tbody) {
		console.error("Không tìm thấy tbody với ID 'donationTableBody'");
		return;
	}
	...
	tbody.innerHTML = "";
	if (sliced.length === 0 && filteredDonations.length === 0) {
		tbody.innerHTML = `<tr><td colspan="5" class="text-center">Không tìm thấy kết quả phù hợp</td></tr>`;
	} else {
	...
	console.log("Đã render bảng với trang:", page, "Dữ liệu:", sliced);
}

function renderPagination() {
	const pagination = document.getElementById("pagination");
	if (!pagination) {
		console.error("Không tìm thấy phần tử với ID 'pagination'");
		return;
	}
	...
	if (pageCount === 0) {
		pagination.innerHTML = `<li class="page-item disabled"><a class="page-link">Không có dữ liệu</a></li>`;
		return;
	}
	...
	console.log("Đã render phân trang với số trang:", pageCount);
}

// ----------- On Load ----------
document.addEventListener("DOMContentLoaded", () => {
	console.log("DOM đã sẵn sàng - script.js");

	// Gắn sự kiện tìm kiếm
	const searchInput = document.getElementById("searchInputDonation");
	if (searchInput) {
		searchInput.addEventListener("input", handleSearch);
		console.log("Đã gắn sự kiện input cho searchInputDonation");
	} else {
		console.error("Không tìm thấy input với ID 'searchInputDonation' khi gắn sự kiện");
	}
